...
parser <- add_option(parser, c("-i", "--indel-score"),  type="integer", default=80,
...
parser <- add_option(parser, c("--min-del-len"),  default=50,
                     type="integer", help="minimum deletion length to call [default %default]")
...
parser <- add_option(parser, c("-b", "--blast-ref"), type="character",
                     default="~/scramble/cluster_analysis/resources/blast_refs/chr2.fa",
                     dest="blast.ref", help="blast reference file (fasta format) [default %default]")
## what to evaluate
...
if(deletions){
  ...
  if(nrow(del.winners) > 0){
    source('make.vcf.R')
    write.scramble.vcf(blastRef, del.winners)
  }
